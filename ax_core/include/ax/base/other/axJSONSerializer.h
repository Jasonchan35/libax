...
public:
	...
	const char*	getEndString( const char* sz ) {
		for( ; *sz; sz++ ) {
...
axStatus ax_json_serialize_io( axJSONSerializer &s, axTinyList<T> &v ) {
	axStatus st;
 //tony
	return -99001;
}
...
template< class T > inline
axStatus ax_json_serialize_io( axJSONDeserializer &s, axTinyList<T> &v ) {
	axStatus st;
 //tony
	return -99002;
}
...
axStatus ax_json_serialize_io( axJSONSerializer &s, axIString_<T> &v ) {
	axStatus st;
	axTempStringA str;
	st = ax_encodeJSON( str, v ); if( !st ) return st;
 	st = s.str_->appendFormat( "\"{?}\"", str ) ;		if( !st ) return st;
	return 0;
}
...
axStatus ax_json_serialize_io( axJSONDeserializer &s, axIString_<T> &v ) {
	...
	st = v.resize(0);			if( !st ) return st;
	st = ax_decodeJSON( v, str ); if( !st ) return st;
